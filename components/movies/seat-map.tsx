...
export function SeatMap({ seats, onSeatToggle }: SeatMapProps) {
  // Dynamically extract rows and columns from API data
  const { rows, leftColumns, rightColumns } = useMemo(() => {
    ...
    // Split columns into left and right sections (for aisle)
  ...
  // Show loading/empty state if no seats
  if (seats.length === 0) {
    return (
  ...
  return (
    <div className="bg-white rounded-xl p-6 sm:p-8 border border-neutral-300">
      ...
      <div className="mb-8 sm:mb-12">
        <div className="w-full h-2 bg-linear-to-r from-transparent via-black to-transparent rounded-full mb-2" />
